...
```hacksidian-details
> Зачем
Рубрика над материалом или статус.

> Как работает
Параметры на карточке: регистр и расстояние между буквами. Значения сохраняются в CSS приёма; для включённого оформления используется кнопка «Обновить уже существующий стиль».

Регистр меняется визуально, исходное имя тега сохраняется.
```
