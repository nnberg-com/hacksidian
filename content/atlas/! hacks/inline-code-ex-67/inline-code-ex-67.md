---
tags:
  - hacksidian_technique
  - hacksidian_inline-code
title: Неоновый код
category: inline-code
sources:
  - https://developer.mozilla.org/en-US/docs/Web/CSS/text-shadow
format: markdown
themes: []
---

```hacksidian-id
inline-code-ex-67
```

```hacksidian-live
inline-code-ex-67
```

```hacksidian-details
> Зачем
Для атмосферной тёмной вставки с редкими короткими словами.

> Как работает
Несколько text-shadow создают свечение вокруг настоящих символов.

> Ограничения
Это декоративный вариант, а не проверка реального состояния системы.
```

```hacksidian-sources
inline-code-ex-67
```

- В исходном обзоре примеры GitHub, Bootstrap и Tailwind адаптировались к обычному `code`; остальные сочетания составлены для атласа. Ссылки на CSS-механизмы не означают, что рецепт заимствован у этих проектов.


```hacksidian-files
inline-code-ex-67
```
