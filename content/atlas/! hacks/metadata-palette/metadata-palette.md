...
tags:
  ...
  - hacksidian_metadata
title: Светлая и тёмная карточка
category: metadata
sources:
...
```hacksidian-details
> Зачем
Проверять оформление на разных фонах без жёстко заданного белого цвета.

> Как работает
Стили используют семантические переменные. Во втором учебном контейнере эти переменные переопределены локально.
