...
> Как работает
::selection задаёт фон и цвет выделенного пользователем текста. Параметр «Цвет выделения» выбирает один из восьми семантических цветов: красный, оранжевый, жёлтый, зелёный, cyan, синий, фиолетовый, розовый. Фиолетовый — только значение по умолчанию; для запроса «при выделении текст на зелёном фоне» выбирается зелёный. Это выделение мышью или клавиатурой, а не постоянная подсветка Markdown ==текста==.

Text selection background: configurable semantic color, including green.
