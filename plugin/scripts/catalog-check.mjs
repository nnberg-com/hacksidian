...
const out=path.join(await mkdtemp(path.join(tmpdir(),'hacksidian-catalog-check-')),'check.mjs');
await esbuild.build({entryPoints:[new URL('./catalog-check.ts',import.meta.url).pathname],bundle:true,platform:'node',format:'esm',outfile:out,banner:{js:"import {createRequire} from 'node:module';const require=createRequire(import.meta.url);"},
