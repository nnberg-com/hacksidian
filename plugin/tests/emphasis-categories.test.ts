...
test('emphasis has descriptive titles and separates deletion from compound markup',()=>{
 ...
 for(let n=1;n<=37;n++){
  const id='emphasis-s'+String(n).padStart(2,'0'),dir=path.join(atlas,'! hacks',id);
  ...
  const group=[28,29,30,31,37].includes(n)?'strikethrough':[32,33,34,35].includes(n)?'emphasis-combinations':'emphasis';
 ...
 expect(counts).toEqual({emphasis:28,strikethrough:5,'emphasis-combinations':4});
 for(const group of ['strikethrough','emphasis-combinations']){
