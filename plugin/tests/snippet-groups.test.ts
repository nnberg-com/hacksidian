...
test('24 unique groups; shared palette but independent quote and callout parameters',()=>{
 expect(new Set(groupManifest.modules.map(m=>m.group)).size).toBe(24);
...
test('migrates working values, removes old files, is idempotent; regrouped Undo restores multiple files',async()=>{
 ...
 try{
  ...
  expect(await migrateSnippetGroups(dir)).toHaveLength(24);
