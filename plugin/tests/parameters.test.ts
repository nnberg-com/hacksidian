...
const ids = ['quote-dashed', 'text-dropcap-accent', 'quote-corner-marks', 'text-system',
...
test('changes only one value, retains Doxygen defaults, and rejects concurrent overwrites', () => {
  const css = read('text-dropcap-accent'), p = readParameters(css)[0];
...
test('editing source does not mutate applied style; explicit update replaces only its block', () => {
  const css = read('text-dropcap-accent');
  const hack = { id: 'text-dropcap-accent', title: 'Drop cap', path: 'atlas/! hacks/text-dropcap-accent/text-dropcap-accent.md', spec: { format: 2, target: 'g-text', hasCss: true }, css };
