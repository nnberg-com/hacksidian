...
test('changes only one value, retains Doxygen defaults, and rejects concurrent overwrites', () => {
  const css = read('note-dropcap'), p = readParameters(css)[0];
...
test('editing source does not mutate applied style; explicit update replaces only its block', () => {
  const css = read('note-dropcap');
  const hack = { id: 'note-dropcap', title: 'Drop cap', path: 'atlas/! hacks/note-dropcap/note-dropcap.md', spec: { format: 2, target: 'g-text', hasCss: true }, css };
