...
const directory=new URL('../../content/atlas/! hacks/palette-semantic-palette-custom/',import.meta.url);
...
it('exposes exactly the eight named Obsidian colors as validated color inputs',()=>{
 ...
 expect(compileHack({id:'palette-semantic-palette-custom',title:'Palette',path:'card.md',spec:JSON.parse(readFileSync(new URL('hack.json',directory),'utf8')),css})).toBe(css);
});
