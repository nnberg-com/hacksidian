  ...
  onload(): void {
    ...
    const render = () => {
      ...
      for (const path of paths) {
        ...
        const remove = row.createEl('button', { cls: 'hacksidian-favourite-toggle', attr: { 'aria-label': en ? `Remove ${title} from favourites` : `Убрать «${title}» из избранного`, title: en ? 'Remove from favourites' : 'Убрать из избранного' } });
        setIcon(remove, 'star');
