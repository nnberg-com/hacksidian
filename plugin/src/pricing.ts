...
function money(cell: string): number {
  ...
  if (!match) throw new Error('Цена не найдена в официальной таблице.');
...
function row(text: string, label: string): string[] {
  ...
  if (!result) throw new Error(`Строка тарифа «${label}» не найдена.`);
  return result;
}
...
export function pricingSource(provider: ProviderId, model: string): string {
  if (!/^[a-zA-Z0-9._-]+$/.test(model)) throw new Error('Укажите ID модели для загрузки тарифа.');
...
export function parsePricing(provider: ProviderId, model: string, text: string, now = new Date()): PricingQuote {
  ...
  if (provider === 'openai' || provider === 'xai') {
    if (!text.includes('`' + model + '`')) throw new Error('Страница тарифа относится к другой модели.');
    const section = text.split('## Pricing')[1]?.split('\n## ')[0];
    if (!section) throw new Error('Раздел тарифов не найден.');
    ...
    if (provider === 'openai') {
      ...
      if (/prompts with|long.context/i.test(section) && !tier) throw new Error('Не удалось разобрать тариф длинного контекста.');
    } else if (/≥/.test(section)) {
      const threshold = section.match(/≥\s*([\d,]+)k/);
      if (!threshold) throw new Error('Не удалось разобрать порог длинного контекста.');
    ...
    if (!names[model]) throw new Error('Для этого ID Claude автоматический тариф пока не распознаётся.');
    ...
    if (!table || !table.includes('Paid Tier')) throw new Error('Стандартный платный тариф этой модели Gemini не найден.');
    ...
    const rate = (label: string): number => {
      ...
      if (!cell) throw new Error(`Тариф Gemini «${label}» не найден.`);
      const lines = cell.split('\n').filter(l => l.includes('$') && !/audio|per hour|storage/i.test(l));
      if (lines.some(l => /prompt|200k|128k|≤|&gt;|&le;/.test(l))) throw new Error('Для этого тарифа Gemini требуется поддержка диапазонов контекста.');
      ...
      if (dated.length) {
        for (const line of dated) {
          ...
          if (!date) throw new Error('Не удалось разобрать срок действия тарифа.');
        ...
        throw new Error('Тариф не действует на текущую дату.');
      }
      if (lines.length !== 1) throw new Error('Официальный тариф неоднозначен.');
      return money(lines[0]);
    };
    prices = { input: rate('Input price'), cached: rate('Context caching price'), output: rate('Output price') };
  }
  if (Object.values(prices).some(p => !Number.isFinite(p) || p < 0)) throw new Error('Некорректные цены в источнике.');
