...
[Настройка параметров через чат на странице приёма](changes/chat-technique-parameters/proposal.md): реализован отдельный путь изменения значений с валидацией и защитой контекста; реальная LLM/UI-проверка остаётся открытой.

Отложенная возможность: [управление стандартными настройками Obsidian через чат](changes/control-obsidian-settings-from-chat/proposal.md). Реализацию не начинать до отдельного поручения.

Отложенное исследование: [общие CSS Variables внутри каждой категории](changes/unify-category-css-variables/proposal.md). Все общие параметры должны работать во всех готовых решениях своей категории; архитектура, хранение значений и разделение базовых решений/уточнений пока не выбраны. Реализацию не начинать до отдельного поручения.
