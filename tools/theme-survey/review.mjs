...
for(const t of all){
 ...
 const checks=[
  ...
  ['note-selection',r=>r.selector.includes('::selection')&&r.declarations.some(d=>/^background/.test(d.prop)&&!/none|transparent/.test(d.value))],
  ...
  ['callout-margin',r=>/\.callout\b/.test(r.selector)&&r.declarations.some(d=>d.prop==='float'&&d.value==='right')],
  ['emphasis-s33',r=>/(?:\bem\s*>\s*em\b|\bstrong\s*>\s*strong\b)/.test(r.selector)&&r.declarations.some(d=>d.prop==='text-decoration'&&/underline/.test(d.value))],
  ['emphasis-s37',r=>/(?:\bdel\s*>\s*mark\b|\bmark\s*>\s*del\b)/.test(r.selector)&&r.declarations.some(d=>d.prop==='filter'&&/blur\(/.test(d.value))],
  ...
  ['note-quote-line',r=>/\bblockquote\b/.test(r.selector)&&r.declarations.some(d=>d.prop==='border-left'&&!/^(?:0|none)\b/.test(d.value))],
  ['note-code-scroll',r=>/\bpre\b/.test(r.selector)&&r.declarations.some(d=>d.prop==='overflow-x'&&/^(?:auto|scroll)$/.test(d.value))],
