...
for(const [id,original] of Object.entries(backup)){
 ...
 for(const lang of ['ru','en']){const p=path.join(dir,`Description.${lang}.md`);if(!fs.existsSync(p))continue;let s=fs.readFileSync(p,'utf8');const heading=lang==='ru'?'Опора на стандартную тему':'Default-theme baseline';s=s.replace(new RegExp('\\n## '+heading+'\\n[\\s\\S]*?(?=\\n## |$)'),'');s+='\n## '+heading+'\n\n'+data.summary[lang]+'\n\n'+(lang==='ru'?'[CSS стандартной темы](./Default.css) · [Данные привязки и нерешённые случаи](./baseline.json)':'[Default theme CSS](./Default.css) · [Bindings and unresolved cases](./baseline.json)')+'\n\n'+data.restore[lang]+'\n';fs.writeFileSync(p,s);}
...
for(const r of report.filter(r=>r.review.length||r.status==='needs-font-decision'))md+=`- [${r.id}](../!%20hacks/${r.id}/Description.ru.md): ${[...new Set(r.review.map(x=>x.kind))].join(', ')||'font'} ([подробности](../!%20hacks/${r.id}/baseline.json)).\n`;
