...
| Маркированные списки (`unordered`) | `hacksidian-11-unordered.css` |
| Нумерованные списки (`ordered`) | `hacksidian-25-ordered.css` |
| Задачи (`task`) | `hacksidian-12-task.css` |
| Псевдозадачи (`taskplus`) | `hacksidian-13-taskplus.css` |
...
Зачёркивание (`strike`) использует `hacksidian-23-strike.css`, сочетания выделений (`combinations`) — `hacksidian-24-combinations.css`. Ранее включённые блоки переносятся из emphasis только при явном обновлении приёма. Старые ID и ссылки карточек сохранены, видимые названия не содержат номеров.

Категория `note` расформирована: приёмы распределены по предметным категориям, оформление целой страницы — в `composition`. Номера остальных файлов сохранены.
