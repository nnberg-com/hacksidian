...
Одна группа — один CSS-файл — один набор переменных. Сейчас 25 групп: 22 группы разметки, палитры (`palette`) и две служебные (`interface`, `meta`). Группа `palette` содержит согласованные палитры; включение одной заменяет ранее включённую палитру того же режима. Остальные группы сохраняются. Некоторые базовые файлы пока содержат только место для будущих правил.
...
| Группа | Базовый файл |
| --- | --- |
| Палитры (`palette`) | `hacksidian-00-palette.css` |
...
Категория `composition` («Композиции») использует `hacksidian-22-composition.css` и содержит приёмы со специальным синтаксисом callout. Категория `callout` оформляет стандартные выноски. При обновлении плагин добавляет новый модуль без перезаписи существующего CSS. Ранее включённый приём переносится из `callout` в `composition` только по команде «Обновить установленный приём».

Зачёркивание (`strikethrough`) использует `hacksidian-23-strikethrough.css`, сочетания выделений (`emphasis-combinations`) — `hacksidian-24-emphasis-combinations.css`. Ранее включённые блоки переносятся из emphasis только при явном обновлении приёма. Старые ID и ссылки карточек сохранены, видимые названия не содержат номеров.
