...
The playground contains ten Russian articles, `p001–p010`, directly in `content/playground/`, plus their SVG attachment. English versions and introductory/service pages are removed. Navigation shows these articles in either interface language. Older user playgrounds with language subfolders still use `contentLanguage`.
...
The second command accepts an optional playground root. It checks all 10 articles and local links without modifying files. Unit tests cover language fallback, independent content selection, dictionary key/parameter parity, language changes in helpers, and chat instructions. They make no billable LLM calls.
