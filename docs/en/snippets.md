...
The plugin creates 23 empty CSS files, one per group. They contain no rules or variable values, so enabling them does not change Obsidian or notes, including notes with `cssclasses: callmered-coloring`. Styling is added only when the user applies a technique or edits a snippet.
...
| Unordered lists (`unordered`) | `hacksidian-11-unordered.css` |
| Ordered lists (`ordered`) | `hacksidian-25-ordered.css` |
| Tasks (`task`) | `hacksidian-12-task.css` |
| Pseudo tasks (`taskplus`) | `hacksidian-13-taskplus.css` |
...
Strikethrough techniques use `hacksidian-23-strike.css`; combined emphasis techniques use `hacksidian-24-combinations.css`. Existing blocks move from emphasis only on explicit technique update.

The `note` category has been retired: techniques now belong to their element categories, and page layouts to `composition`. Other filenames retain their original numbers.
